  ...
  setup(__props, { emit }) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        __props.withButton ? (openBlock(), createElementBlock("div", {
          key: 0,
...
const useWindowSizeLifecycles = (root) => {
  ...
  function getWindowSize() {
    if (root.value) {
      windowHeight.value = root.value.parentElement ? root.value.parentElement.clientHeight : root.value.clientHeight;
...
const _hoisted_1$1 = ["onTouchstart"];
const _sfc_main$2 = /* @__PURE__ */ defineComponent({
  ...
  emits: ["dragstart", "drag", "click", "dragend"],
  setup(__props, { emit }) {
    const props = __props;
    ...
    let zIndex = ref(1);
    onMounted(() => {
      self2.value.addEventListener("transitionend", () => {
        if (!dragging.value) {
          zIndex.value = 1;
        }
      }, false);
    ...
    const left = computed(() => dragging.value ? shiftX.value : props.rowShift + props.sort % props.columnCount * props.cellWidth);
    const top = computed(() => dragging.value ? shiftY.value : Math.floor(props.sort / props.columnCount) * props.cellHeight);
    const style = computed(() => {
      return {
        zInder: zIndex.value,
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ...
        onTouchstart: withModifiers(mousedown, ["stop"])
      }, [
        renderSlot(_ctx.$slots, "default")
      ], 46, _hoisted_1$1);
    };
...
const _sfc_main$1 = /* @__PURE__ */ defineComponent({
  props: {
    ...
    autoCellSize: { default: true },
    cellWidth: { default: 80 },
    cellHeight: { default: 80 },
    cellMinWidth: { default: 80 },
    cellMinHeight: { default: 80 },
    draggable: { default: false },
    dragDelay: { default: 0 },
    sortable: { default: false },
    center: { default: true },
    columnCountMax: { default: -1 },
    rowCountMax: { default: -1 }
  },
  emits: ["remove", "drag", "dragstart", "dragend", "click", "sort"],
  setup(__props, { emit }) {
    const props = __props;
    const gridContainer = ref();
    const { windowHeight: windowHeight2, windowWidth: windowWidth2, useWindowSizeLifecycles: useWindowSizeLifecycles2 } = windowSize();
    onMounted(() => useWindowSizeLifecycles2(gridContainer));
    const list = ref([]);
    ...
    const gridResponsiveWidth = computed(() => {
      if (props.gridWidth < 0) {
        return windowWidth2.value;
      } else {
        return Math.min(windowWidth2.value, props.gridWidth);
      }
    });
    const gridResponsiveHeight = computed(() => {
      if (props.gridHeight < 0) {
        return windowHeight2.value;
      } else {
        return Math.min(windowHeight2.value, props.gridHeight);
      }
    });
    ...
    const columnCount = computed(() => {
      const columnCount2 = Math.floor(gridResponsiveWidth.value / cellWidth.value);
      if (props.columnCountMax > 0)
        return Math.min(columnCount2, props.columnCountMax);
      else
        return Math.min(columnCount2, list.value.length);
    });
    const rowShift = computed(() => {
      if (props.center) {
        const contentWidth = cellWidth.value * columnCount.value;
        const rowShift2 = contentWidth < gridResponsiveWidth.value ? (gridResponsiveWidth.value - contentWidth) / 2 : gridResponsiveWidth.value / columnCount.value / 2;
        return Math.floor(rowShift2);
      }
      return 0;
    });
    function wrapEvent(gridItemEvent) {
    ...
    function getListClone() {
      return list.value.slice(0).sort((a, b) => {
        return a.sort - b.sort;
      });
    }
    function removeItem({ index }) {
      let removeItem2 = list.value.find((v) => v.index === index);
      let removeItemSort = removeItem2.sort;
      list.value = list.value.filter((v) => {
        return v.index !== index;
      }).map((v) => {
        let sort = v.sort > removeItemSort ? v.sort - 1 : v.sort;
        return __spreadProps(__spreadValues({}, v), { sort });
      });
    ...
    function onDrag(event) {
      ...
      emit("drag", __spreadValues({
        datetime: Date.now(),
        items: getListClone()
      }, event));
    }
    function sortList(itemIndex, gridPosition) {
    ...
    return (_ctx, _cache) => {
      return openBlock(), createElementBlock("div", {
        ref: (_value, _refs) => {
          _refs["gridContainer"] = _value;
          gridContainer.value = _value;
        },
        class: "v-grid",
        style: normalizeStyle(unref(style))
      }, [
        (openBlock(true), createElementBlock(Fragment, null, renderList(list.value, (v) => {
          return openBlock(), createBlock(_sfc_main$2, {
            ...
            "drag-delay": __props.dragDelay,
            "column-count": unref(columnCount),
            "cell-width": unref(cellWidth),
            "cell-height": unref(cellHeight),
            "window-width": unref(windowWidth2),
            "row-shift": unref(rowShift),
            ...
            _: 2
          }, 1032, ["index", "sort", "draggable", "drag-delay", "column-count", "cell-width", "cell-height", "window-width", "row-shift"]);
        }), 128))
      ], 4);
    };
  }
...
const _hoisted_2 = { class: "color-header" };
const _hoisted_3 = /* @__PURE__ */ createTextVNode(" Resizable Grid Dashboard ");
const _hoisted_4 = { class: "test" };
const _sfc_main = /* @__PURE__ */ defineComponent({
  setup(__props) {
    const colors = ref(generateRGBColors(8));
    const selected = ref(colors.value[0]);
    function click({ items, index }) {
          ...
          createVNode(_sfc_main$1, {
            ...
            cell: withCtx((props) => [
              createVNode(_sfc_main$3, {
                ...
                index: props.index,
                "with-button": true,
                onRemove: ($event) => props.remove()
              }, null, 8, ["color", "index", "onRemove"])
            ]),
            _: 1
          }, 8, ["items"])
        ])
