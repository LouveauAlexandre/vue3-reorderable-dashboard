...
<script lang="ts" setup>
  ...
  const cellWidth = computed(() => {
    if (props.autoCellSize) {
      return Math.max(
        Math.ceil(gridResponsiveWidth.value / props.columnCountMax) - 1,
        props.cellMinWidth,
      );
    } else {
  ...
  const cellHeight = computed(() => {
    if (props.autoCellSize) {
      return Math.max(
        gridResponsiveHeight.value /
          Math.ceil(list.value.length / props.rowCountMax) -
          1,
        props.cellMinHeight,
      );
    } else {
  ...
  const height = computed(
    () => Math.ceil(list.value.length / columnCount.value) * cellHeight.value,
  );
  ...
  const columnCount = computed(() => {
    ...
    else return Math.min(columnCount, list.value.length);
  });
  ...
  const rowShift = computed(() => {
    if (props.center) {
      ...
      const rowShift =
        contentWidth < gridResponsiveWidth.value
